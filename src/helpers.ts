	...
	switch (typeIdentifier.type) {
		...
		case "type_identifier":
			if (
				...
				typeof JAVA_WRAPPER_CLASSES_TYPES_TABLE[typeIdentifier.text] ===
					"string"
			) {
				...
				// the same package. so...
				if (
					!options.sameScopeTypeDeclarations.includes(
						typeIdentifier.text,
					)
				) {
					// or imported?
					if (
						typeIdentifier.text in options.imports &&
						options.imports[typeIdentifier.text] != null
					) {
						const { identifier, scope } =
							options.imports[typeIdentifier.text]!;

						if (scope === "") return [identifier];

						if (
							scope in TYPE_IDENTIFIERS_IMPORTS &&
							TYPE_IDENTIFIERS_IMPORTS[scope] != null
						) {
							if (
								identifier in TYPE_IDENTIFIERS_IMPORTS[scope] &&
								TYPE_IDENTIFIERS_IMPORTS[scope][identifier] !=
									null
							) {
								return [
									[
										...TYPE_IDENTIFIERS_IMPORTS[scope][
											identifier
										],
										identifier,
									].join("."),
								...
								throw new Error(
									"expected type identifer to be specified",
								);
							}
						}
						...
						console.log(
							nodePosition(typeIdentifier, options.currentPath),
						);
						console.log(typeIdentifier.text);
						throw new Error(
							"Expected the type identifier to be a class in the same package scope",
						);
					}
				}
		...
		case "generic_type":
			...
			if (
				...
				const typeArguments = typeIdentifier
					.child(1)!
					.children.filter((_, i) => i % 2 === 1)
					.map((child) => resolveTypeIdentifier(child, options))
					.flat();

				if (
					UNWRAPPABLE_CLASSES.includes(
						JAVA_GENERIC_CLASSES_TABLE[type],
					)
				) {
				...
				throw new Error(
					"Unhandled generic type in generic classes, must add",
				);
			}

		case "scoped_type_identifier":
			const [scope, ...subTypes] = typeIdentifier.children.filter(
				(t) => t.type !== ".",
			);
			const scopeText = scope!.text;

			if (
				scopeText in options.imports &&
				options.imports[scopeText] != null
			) {
				...
				if (scopeImport.scope in SCOPE_DECLARATIONS) {
					...
					for (const scopeSegment of [scope!, ...subTypes]) {
						if (
							scopeSegment.text in parent &&
							parent[scopeSegment.text] != null
						) {
							resolved.push(
								resolveTypeIdentifier(scopeSegment, {
...
export function getImports(tree: Parser.Tree) {
	...
	for (const decl of tree.rootNode.descendantsOfType("import_declaration")) {
		...
		if (identifier == null) throw new Error("unwanted identifier");
			...
			if (
				...
				for (const argument of args.children) {
					if (argument.type === "(" || argument.type === ")") {
						...
						} else if (object.text === "ClientCookie") {
							// im 100% sure that this is not the fix for this cuz i wrote the code.
							// but im 100% lost on what any of this really did.
							if (field.text === "COMMENT_ATTR") {
								annotation.arguments.push({
									type: "string_literal",
									value: "comment", // https://hc.apache.org/httpcomponents-client-4.5.x/current/httpclient/apidocs/constant-values.html#org.apache.http.cookie.ClientCookie.COMMENT_ATTR
								});
							} else {
								console.log(
									nodePosition(object, options.currentPath),
								);
...
export function parseParameterAnnotationModiifer(
	modifier: Annotation,
): ParameterAnnotationModifier {
	if (modifier.type === "annotation") {
		...
		if (arg.type === "string_literal") {
			if (modifier.name === "Field") {
				...
				throw new Error(
					"unhandled annotation modifier type (string_literal)",
				);
			}
			...
			if (modifier.name === "FieldMap") {
				...
				console.log(modifier);
				throw new Error(
					"unhandled annotation modifier type (boolean_literal)",
				);
			}
		} else {
